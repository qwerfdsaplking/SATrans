...
class MetaPositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, hidden_dim, num_domains, dropout=0.1,use_norm=True,meta_dnn_hidden_units=(32,64,32)):
        ...
        if self.use_norm:
            self.ffn_layer_norms = nn.ModuleList([nn.LayerNorm(hidden_dim, eps=1e-6) for _ in range(num_domains)])
...
class Meta_Transformer_Layer(nn.Module):
    def __init__(self, embedding_size,num_domains,meta_dnn_hidden_units,flag, mode='Q',head_num=2, use_res=True, scaling=True, seed=1024, device='cpu'):
        ...
        if 'nometanorm' in flag:
            ...
            self.meta_mlp = MetaPositionwiseFeedForward(hidden_dim=embedding_size,num_domains=num_domains,use_norm=False,meta_dnn_hidden_units=meta_dnn_hidden_units)
        else:
            self.meta_mlp = MetaPositionwiseFeedForward(hidden_dim=embedding_size,num_domains=num_domains,use_norm=True,meta_dnn_hidden_units=meta_dnn_hidden_units)
...
class Gate_Transformer_Layer(nn.Module):
    def __init__(self, embedding_size,num_domains,flag, mode='QK',head_num=4, use_res=True, scaling=True, seed=1024, device='cpu'):
    ...
    def forward(self, inputs,  Gate_Q=None, Gate_K=None, Gate_V=None,domain_ids=None):
        ...
        if 'Q' in self.mode:
            querys = querys*Gate_Q.unsqueeze(1).expand(querys.shape)
        if 'K' in self.mode:
            keys = keys*Gate_K.unsqueeze(1).expand(keys.shape)
        if 'V' in self.mode:
            values = values*Gate_V.unsqueeze(1).expand(values.shape)
        # head_num None F D/head_num
        querys = torch.stack(torch.split(querys, self.att_embedding_size, dim=2))
        keys = torch.stack(torch.split(keys, self.att_embedding_size, dim=2))
        values = torch.stack(torch.split(values, self.att_embedding_size, dim=2))
...
class Starv3_Trans(BaseModel):
    def __init__(self, linear_feature_columns, dnn_feature_columns, domain_column,
        ...
        #meta_dnn_hidden_units=self.flag.split('xxx')[1].split('-')
        ...
        if 'metabias' in self.flag:
            #meta_param_size = embedding_size*embedding_size*2*2 + embedding_size*3
            meta_param_size=sum([meta_dnn_hidden_units[i]*meta_dnn_hidden_units[i+1] for i in range(len(meta_dnn_hidden_units)-1)])
            meta_param_size+=sum(meta_dnn_hidden_units[1:])
        else:
            #meta_param_size = embedding_size * embedding_size * 2 * 2
            meta_param_size=sum([meta_dnn_hidden_units[i]*meta_dnn_hidden_units[i+1] for i in range(len(meta_dnn_hidden_units)-1)])

        if 'large' in flag:
            map_dnn_size =  [embedding_size*2, meta_param_size]
        else:
    ...
    def forward(self, X):
        ...
        if 'layerid' not in self.flag:
            ...
            for i in range(len(self.meta_dnn_hidden_units)-1):
                #print(offset, offset + self.meta_dnn_hidden_units[i] * self.meta_dnn_hidden_units[i + 1])
            ...
            if 'metabias' in self.flag:
                ...
                for dim in self.meta_dnn_hidden_units[1:]:
                    #print(offset, offset + dim)

                    domain_bias = domain_vec[:,offset:offset+dim]
                    offset+=dim
                    domain_bias_list.append(domain_bias)
        ...
        for layerid, layer in enumerate(self.domain_int_layers):
            ...
            if 'qkvid' in self.flag:
                ...
                att_input = layer(att_input,
                                  ...
                                  [domain_weight_list,domain_bias_list],
                                  [domain_weight_list,domain_bias_list],domain_ids=domain_ids_x)
        ...
        if len(dense_value_list)>0:
            dense_input = concat_fun(dense_value_list, axis=1)
            #print(att_output.shape, dense_input.shape)
            stack_out = concat_fun([att_output, dense_input])
        else:
            stack_out = att_output
...
class Gate_Trans(BaseModel):
    def __init__(self, linear_feature_columns, dnn_feature_columns, domain_column,
        ...
        super(Gate_Trans, self).__init__(linear_feature_columns, dnn_feature_columns, l2_reg_linear=0,
                                      l2_reg_embedding=l2_reg_embedding, init_std=init_std, seed=seed, task=task,
                                      device=device, gpus=gpus)
        ...
        if att_layer_type == 'deepctr':
            self.domain_int_layers = nn.ModuleList(
                [Gate_Transformer_Layer(embedding_size,num_domains,flag, meta_mode,
                                        att_head_num, att_res, device=device) for _ in range(domain_att_layer_num)])
        ...
        if 'norelu' in flag:
            self.domain_map_dnn = DNN_v2(domain_dnn_input_dim, [embedding_size])
        else:
            self.domain_map_dnn = DNN(domain_dnn_input_dim, embedding_size)
    ...
    def forward(self, X):
        ...
        #domain_emb = self.embedding_dict[self.domain_column](domain_ids).squeeze()
        #domain_emb = att_input[:, domain_index, :].squeeze()
        if 'times2' in self.flag:
            times=2
        else:
            times=1
        if 'layerid' not in self.flag:
            domain_emb = F.relu(domain_emb)
            domain_gates = F.sigmoid(self.domain_map_dnn(domain_emb))*times
            ...
            if 'layerid' in self.flag:
                ...
                for qkvid in range(3):#QKV
                    ...
                    if 'cat' in self.flag:
                        ...
                        all_emb = F.relu(domain_emb+layerid_emb+qkvid_emb )
                    domain_gates = F.sigmoid(self.domain_map_dnn(all_emb))*times


                    domain_gates_list.append(domain_gates)




            if 'qkvid' in self.flag:
                att_input = layer(att_input, domain_gates_list[0],domain_gates_list[1],domain_gates_list[2])
            elif 'layerid' in self.flag:#不区分qkvid，则都用一样的
                att_input = layer(att_input, domain_gates_list[0],domain_gates_list[0],domain_gates_list[0])
            else:
                att_input = layer(att_input,domain_gates,domain_gates,domain_gates)
...
class Star_Net(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, domain_column, num_domains, domain_id_as_feature=False, att_layer_num=3,
        ...
        self.domain_column = domain_column
        if 'usetranslayers' in self.flag:
            self.int_layers = nn.ModuleList(
